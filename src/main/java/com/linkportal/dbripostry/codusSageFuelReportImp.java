...
public class codusSageFuelReportImp implements codusSageFuelReport {
	...
	private static final String MasterSqlForallInvoice = "SELECT top 3000 scheme.Codisplinvdm.item As Invoice_No,\r\n"
	...
	public List<codusSageFuelInvoice> populateFuelInvoices(HttpServletRequest req) {
		...
		if (!Strings.isNullOrEmpty(req.getParameter("batch"))) {
			sqlForInvoiceList = sqlForInvoiceList + "and scheme.Codisplinvhm.batch like '%" + req.getParameter("batch").trim()+"%'";
		}
		
		if (!Strings.isNullOrEmpty(req.getParameter("invoiceno"))) {
			sqlForInvoiceList = sqlForInvoiceList + "and scheme.Codisplinvdm.item like '%"+ req.getParameter("invoiceno").trim() + "%'";
		}

		if (!Strings.isNullOrEmpty(req.getParameter("financialyear"))) {
			sqlForInvoiceList = sqlForInvoiceList + "and scheme.Codisplinvhm.analysis_codes2 like '%"+req.getParameter("financialyear").trim() + "'";
		} 
		
		if (Strings.isNullOrEmpty(req.getParameter("batch")) && Strings.isNullOrEmpty(req.getParameter("financialyear")) && Strings.isNullOrEmpty(req.getParameter("invoiceno"))) 
		{
